...
def plot_collisions(ray_origins: np.ndarray, ray_directions: np.ndarray, triangles: List[Triangle],
    ...
    ax.set_title('Illustration of some rays generated from emitter')

    # Show plot
    plt.show()


def plot_surface(X: np.ndarray, Y: np.ndarray, Z: np.ndarray, title_):
    """
    ...
    # Add color bar
    fig.colorbar(surf, ax=ax, label='Mean Power')
